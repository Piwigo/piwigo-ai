...
<div class="piwigoai m-5 pb-16">
  <div class="flex flex-col items-start text-start">
    ...
    <div class="mt-4 flex flex-col text-start">
      ...
      <input class="p-ai-input cursor-not-allowed" 
        id="url_server_ai" name="url_server_ai" type="text" 
        value="{$P_AI_CONFIG.url_server_ai}" disabled
      />
    </div>
...
{* <div>
  <p>Available configuration (use pwg.ai.config) : </p>
  <ul>
    <li><b>ai_send_picture_file</b> : True or False - Send an image file instead of the image url. (default
      <b>false</b>)
    </li>
    <li><b>ai_ticket_callback</b> : True or False - If the callback is set to true, the ai server will send the ticket
      data directly. (default <b>true</b>)</li>
    <li><b>ai_description_prefix</b> : String - . (default <b>{'(Generated by Artificial Intelligence)'|translate}</b>)
    </li>
  </ul>
  ...
  <p>
  <ul>
    {foreach from=$P_AI_CONFIG item=config key=name}
      <li><b>{$name}</b> : {if true === $config or false === $config}{($config) ? 'true' : 'false'}{else}{$config}{/if}
      </li>
    {/foreach}
  </ul>

  {if empty($P_AI_CONFIG.account_id)}
    <form method="post">
      <input name="pwg_token" type="hidden" value="{$PWG_TOKEN}" />
      <button class="buttonLike" type="submit" name="create_pwgai_account">Create an account</button>
    </form>
  {/if}
</div> *}